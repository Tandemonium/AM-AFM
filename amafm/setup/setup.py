...
def setup_venv(cuda: str='cu118', venv_name: str='.venv', venv_dir: Optional[str]=None, 
    ...
    if os.name == 'nt':
        print(rf'"{cwd}/setup_venv_windows.sh" {cuda} "{venv_dir}" "{venv_name}" {python_path}')
        os.system(rf'"{cwd}/setup_venv_windows.sh" {cuda} "{venv_dir}" "{venv_name}" {python_path}')
    elif os.name == 'posix':
        os.system(f'sh "{cwd}/setup_venv_posix.sh" {cuda} "{venv_dir}" "{venv_name}" {python_path}')
